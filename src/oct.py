...
from src.solver_monitor import (
    SolverMonitor, combined_callback, configure_solver_logging, solver_record,
)
...
class optimalDecisionTreeClassifier:
    ...
    def __init__(self, max_depth=3, min_samples_split=2, alpha=0, warmstart=True,
                 timelimit=600, output=False, log_file=None, random_state=42,
                 concise_progress=True, heartbeat_seconds=60,
    ...
    def fit(self, x, y):
        ...
        self.last_solver_status = int(m.Status)
        self.solve_history = [solver_record(m, monitor=monitor)]
